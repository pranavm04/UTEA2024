...
double D(struct reb_particle *p) {
    return (p[1].y * p[1].vy) + (p[1].z * p[1].vz);
}
...
double kepler_solver_D_finder(const struct reb_simulation* const r, const struct reb_particle p1, const double M, double _dt) {
    ...
    double p1_y = p1.y + (f*p1.y) + (g*p1.vy);
    double p1_vy = p1.vy + (fd*p1.y) + (gd*p1.vy);
    double p1_z = p1.z + (f*p1.z) + (g*p1.vz);
    double p1_vz = p1.vz + (fd*p1.z) + (gd*p1.vz);

    return (p1_y*p1_vy) + (p1_z*p1_vz);
}
